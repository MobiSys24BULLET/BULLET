...
TcpTxBuffer::GetTypeId()
{
    static TypeId tid = TypeId("ns3::TcpTxBuffer")
                            ...
                            .AddTraceSource("TxBufferSize",
                                            "Current TCP tx buffe size",
                                            MakeTraceSourceAccessor(&TcpTxBuffer::m_bufferSize),
                                            "ns3::TcpTxBuffer::BufferTracedCallback");
    return tid;
}
...
TcpTxBuffer::SetHeadSequence(const SequenceNumber32& seq)
{
    ...
    //std::cout<<Simulator::Now().GetSeconds()<<" transport head: "<<m_firstByteSeq <<std::endl;
}
...
TcpTxBuffer::Add(Ptr<Packet> p)
{
    ...
    if (p->GetSize() <= Available())
    {
        if (p->GetSize() > 0)
        {
            ...
            it2 = m_appList.begin();
            (*it2)->GetPacket()->FindFirstMatchingByteTag(tag);
            m_bufferSize(m_size - m_sentSize, tag.m_frameNum);

            NS_LOG_LOGIC(Simulator::Now().GetSeconds() << " Updated size=" << m_size << ", lastSeq="
...
TcpTxBuffer::GetNewSegment(uint32_t numBytes)
{
    ...
    for (it2 = m_appList.begin(); it2 != m_appList.end(); ++it2){
        (*it2)->GetPacket()->FindFirstMatchingByteTag(tag);
        m_bufferSize(m_size - m_sentSize, tag.m_frameNum);
        count += 1;
        break;
...
TcpTxBuffer::NextSeg(SequenceNumber32* seq, SequenceNumber32* seqHigh, bool isRecovery) const
{
    ...
    if (SizeFromSequence(m_firstByteSeq + m_sentSize) > 0)
    {
        ...
        {
            NS_LOG_INFO(Simulator::Now().GetSeconds()<<" There is unsent data. Send it");
