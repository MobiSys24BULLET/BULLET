...
void TcpCopa::CongControl (Ptr<TcpSocketState> tcb,const TcpRateOps::TcpRateConnection &rc,
    ...
    //std::cout<<Simulator::Now().GetSeconds()<<" RTT: "<< rtt <<" "<< srtt << std::endl;
    ...
    uint64_t delay_us;
    uint32_t packet_size = tcb->m_segmentSize;
    ...
    //NS_LOG_UNCOND(this <<" "<<Simulator::Now().GetSeconds() << " Copa delay_us: " << rttStanding.GetMilliSeconds() << " " <<rtt_min.GetMilliSeconds()<<" "<<cwnd_bytes);
    if(0==delay_us){
        increase_cwnd=true;
    }else{
        ...
        //NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " Copa target and current rate: "<< target_rate << " "<< current_rate);
