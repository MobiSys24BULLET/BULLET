...
/* Get L4 protocol to define congestion control respectively */
Ptr<TcpL4Protocol>
TcpSocketBase::GetTcp()
{
    return m_tcp;
}
...
TcpSocketBase::DoForwardUp(Ptr<Packet> packet, const Address& fromAddress, const Address& toAddress)
{
    ...
    //NS_LOG_UNCOND(Simulator::Now() << " Addr: " << fromAddress << " seq: "<< seq);
...
TcpSocketBase::BytesInFlight() const
{
    ...
    //std::cout<< "Bytes inflight: "<< bytesInFlight << std::endl;
...
{
    std::cout<<Simulator::Now()<<" Data RX"<<std::endl;
    NS_LOG_FUNCTION(this << tcpHeader);
...
TcpSocketBase::ReTxTimeout()
{
    ...
    m_tcb->m_cWnd = m_tcb->m_segmentSize*10; //Need to be checked
    m_tcb->m_cWndInfl = m_tcb->m_cWnd;
...
{
    NS_LOG_UNCOND("Pacing status is " << pacing);
