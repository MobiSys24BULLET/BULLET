    ...
    static TypeId tid =
        TypeId("ns3::LteRlcAm")
            ...
            .AddAttribute("ReportBufferStatusTimer",
                          ...
                          TimeValue(TimeValue(MicroSeconds(1000))),
            ...
            .AddAttribute("MaxTxBufferSize",
                          ...
                          UintegerValue(1.5*1024* 1024),
                          MakeUintegerAccessor(&LteRlcAm::m_maxTxBufferSize),
                          MakeUintegerChecker<uint32_t>())
            .AddTraceSource("RlcBufferTrace",
                          "Trace the size of RLC AM buffer",
                          MakeTraceSourceAccessor(&LteRlcAm::m_currentBufferSize),
...
LteRlcAm::DoReportBufferStatus()
{
    ...
    VideoStreamTag front_tag;
    VideoStreamTag back_tag;

    if(m_txonBufferSize > 0){
        //m_txonBuffer.front().m_pdu->PeekPacketTag(front_tag);
        m_txonBuffer.front().m_pdu->FindFirstMatchingByteTag(front_tag);
        m_txonBuffer.back().m_pdu->FindFirstMatchingByteTag(back_tag);
    }

    r.is_frame = false;
    
    // If current buffer has data of frame
    if (front_tag.m_byteSequenceNum > 0 || back_tag.m_byteSequenceNum > 0){
        r.onDeviceQueueing = (Simulator::Now() - front_tag.m_senderTimestamp).GetSeconds(); 
        r.is_frame = true;
        r.front_frameNum = (int)front_tag.m_frameNum;
        r.back_frameNum = (int)back_tag.m_frameNum;
        r.front_sequence = (int)front_tag.m_currentSequence;
        r.back_sequence = (int)back_tag.m_currentSequence;
        std::cout<<"RLC AM txed front: "<<Simulator::Now().GetSeconds()<<" "<<front_tag.m_frameNum<<" "<<front_tag.m_currentSequence<<" "<<(uint32_t)m_lcid<<" "<<m_txonBufferSize<<std::endl;
        std::cout<<"RLC AM txed back: "<<Simulator::Now().GetSeconds()<<" "<<back_tag.m_frameNum<<" "<<back_tag.m_currentSequence<<" "<<(uint32_t)m_lcid<<" "<<m_txonBufferSize<<std::endl;
...
LteRlcAm::BufferSizeTrace ()
{
  ...
  // TODO: 230208 aggregate all RLC AM LCID buffer traces!!
  //m_current

  m_traceBufferSizeEvent = Simulator::Schedule(MicroSeconds(500), &LteRlcAm::BufferSizeTrace, this);
}
