...
ThreeGppHttpServer::Receive (Ptr<Packet> packet)
{
  ...
  if(m_requestBytesToBeReceived - packet->GetSize () <= 0)
    {
      m_lastPacket = true;
    }
  ...
  if(m_requestBytesToBeReceived < requestSize)
    {
      NS_LOG_UNCOND (this << " The received packet"
      ...
      //NS_LOG_UNCOND (Simulator::Now().GetSeconds() <<" Debug: "<<  m_requestBytesToBeReceived);

      //m_requestBytesToBeReceived -= (requestSize - m_requestBytesToBeReceived);
  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      if(m_requestBytesToBeReceived > 0 || !m_lastPacket)
      {
      ...
      std::cout<<"E2E latency: "<<(Simulator::Now () - httpHeader.GetClientTs ()).GetSeconds()<<" "<<
      httpHeader.GetClientTs ()<<std::endl;
                                            ...
                                            uint32_t objectSize)
{
  ...
  //commented to simulate streaming mode
  NS_LOG_UNCOND(Simulator::Now().GetSeconds()<<" Tx buffer size: " << it->second.txBufferSize);
