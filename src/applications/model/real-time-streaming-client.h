 ...
 * \defgroup http RtsClientServer
 *
 * This traffic generator simulates web browsing traffic using the Hypertext
 * Transfer Protocol (HTTP). It consists of one or more RtsClient
 ...
 * RtsClientHelper and ThreeGppHttpServerHelper for usage instructions.
 *
 ...
 */
class RtsClient : public Application
{
  public:
    /**
     ...
     * \param httpClient Pointer to this instance of RtsClient,
     *                               which is where the trace originated.
     */
    typedef void (*TracedCallback)(Ptr<const RtsClient> httpClient);
  ...
  private:
    ...
    ns3::TracedCallback<Ptr<const RtsClient>> m_connectionEstablishedTrace;
    /// The `ConnectionClosed` trace source.
    ns3::TracedCallback<Ptr<const RtsClient>> m_connectionClosedTrace;
    /// The `Tx` trace source.
    ...
    ns3::TracedCallback<Ptr<const RtsClient>, Ptr<const Packet>> m_rxMainObjectTrace;
    ...
    ns3::TracedCallback<Ptr<const RtsClient>, Ptr<const Packet>> m_rxEmbeddedObjectTrace;
    /// The `Rx` trace source.
