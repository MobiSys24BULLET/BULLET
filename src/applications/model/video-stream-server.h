...
class Packet;
  ...
  public:
    ...
    typedef void (*RespondFrameNumCallback)(uint32_t frameNum, Time dl);
  ...
  private:
     ...
     */
    void SendPacket (ClientInfo *client, uint32_t packetSize, uint32_t remainToSend);
    ...
    void TraceSocket (Ptr<Socket> sock);
    void CwndChange (uint32_t oldCwnd, uint32_t newCwnd);  
    void AckRx (SequenceNumber32 oldSeq, SequenceNumber32 newSeq);
    void InflightChange (uint32_t oldInflight, uint32_t newInflight);
    void SocketBufferChange (int bufSize, int frameNum);
