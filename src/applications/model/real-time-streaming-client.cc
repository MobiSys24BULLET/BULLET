...
#include "real-time-streaming-client.h"
...
NS_LOG_COMPONENT_DEFINE("RtsClient");

namespace ns3
{

NS_OBJECT_ENSURE_REGISTERED(RtsClient);
...
// static
std::string
RtsClient::GetStateString(RtsClient::State_t state)
{
    switch (state)
    {
    case NOT_STARTED:
...
void
RtsClient::ConnectionSucceededCallback(Ptr<Socket> socket)
{
    ...
    if (m_state == CONNECTING)
    {
        ...
        socket->SetRecvCallback(MakeCallback(&RtsClient::ReceivedDataCallback, this));
        NS_ASSERT(m_embeddedObjectsToBeRequested == 0);
        m_eventRequestMainObject =
            Simulator::ScheduleNow(&RtsClient::RequestMainObject, this);
    }
    else
    {
...
void
RtsClient::ConnectionFailedCallback(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
...
void
RtsClient::NormalCloseCallback(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
...
void
RtsClient::ErrorCloseCallback(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
...
void
RtsClient::ReceivedDataCallback(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
...
RtsClient::OpenConnection()
{
        ...
        m_state == PARSING_MAIN_OBJECT || m_state == READING)
    {
        ...
        m_socket->SetConnectCallback(
            MakeCallback(&RtsClient::ConnectionSucceededCallback, this),
            MakeCallback(&RtsClient::ConnectionFailedCallback, this));
        m_socket->SetCloseCallbacks(MakeCallback(&RtsClient::NormalCloseCallback, this),
                                    MakeCallback(&RtsClient::ErrorCloseCallback, this));
        m_socket->SetRecvCallback(MakeCallback(&RtsClient::ReceivedDataCallback, this));
...
void
RtsClient::RequestEmbeddedObject()
{
    NS_LOG_FUNCTION(this);
...
void
RtsClient::ReceiveMainObject(Ptr<Packet> packet, const Address& from)
{
...
void
RtsClient::ReceiveEmbeddedObject(Ptr<Packet> packet, const Address& from)
{
    NS_LOG_FUNCTION(this << packet << from);
        ...
        else
        {
            ...
            if (m_embeddedObjectsToBeRequested > 0)
            {
                ...
                m_eventRequestEmbeddedObject =
                    Simulator::ScheduleNow(&RtsClient::RequestEmbeddedObject, this);
            }
            else
            {
                /*
...
void
RtsClient::Receive(Ptr<Packet> packet)
{
...
RtsClient::EnterParsingTime()
{
    ...
    if (m_state == EXPECTING_MAIN_OBJECT)
    {
        ...
        m_eventParseMainObject =
            Simulator::Schedule(parsingTime, &RtsClient::ParseMainObject, this);
...
RtsClient::EnterReadingTime()
{
    ...
    if (m_state == EXPECTING_EMBEDDED_OBJECT || m_state == PARSING_MAIN_OBJECT)
    {
        ...
        m_eventRequestMainObject =
            Simulator::Schedule(readingTime, &RtsClient::RequestMainObject, this);
...
void
RtsClient::CancelAllPendingEvents()
{
    NS_LOG_FUNCTION(this);
...
void
RtsClient::SwitchToState(RtsClient::State_t state)
{
    ...
    m_state = state;
    NS_LOG_INFO(this << " RtsClient " << oldState << " --> " << newState << ".");
