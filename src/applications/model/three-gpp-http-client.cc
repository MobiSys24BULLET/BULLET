...
ThreeGppHttpClient::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::ThreeGppHttpClient")
    ...
    .AddAttribute ("Streaming",
                   "Enable streaming mode",
                   BooleanValue (true),
                   MakeBooleanAccessor (&ThreeGppHttpClient::m_streamingMode),
                   MakeBooleanChecker ())
    .AddAttribute ("fps",
                   "Set frame per second",
                   UintegerValue (30),
                   MakeUintegerAccessor (&ThreeGppHttpClient::m_fps),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("bitrate",
                   "Set bitrate",
                   UintegerValue (20*1024*1024), // default: 20Mbps
                   MakeUintegerAccessor (&ThreeGppHttpClient::m_bitrate),
                   MakeUintegerChecker<uint16_t> ())
...
ThreeGppHttpClient::ServeNewRequest (ThreeGppHttpHeader::ContentType_t type)
{
  ...
  NS_LOG_UNCOND (Simulator::Now().GetSeconds() << " Request to be served is " << requestSize/1024 << " KB, actual sent is "
          << actualSent/1024<< " KB " << type);
...
ThreeGppHttpClient::ServeFromTxBuffer ()
{
  ...
  {
    NS_LOG_UNCOND (Simulator::Now().GetSeconds() << " Has header1");
  ...
  if (actualBytes == static_cast<int> (packetSize))
  {
    NS_LOG_UNCOND (Simulator::Now().GetSeconds() << " Has header2 "<< actualBytes);
...
void
ThreeGppHttpClient::StartStreaming ()
{
  ServeNewRequest(ThreeGppHttpHeader::EMBEDDED_OBJECT);
  Simulator::Schedule(MilliSeconds(1000/m_fps),
              &ThreeGppHttpClient::StartStreaming,
              this);
}
...
ThreeGppHttpClient::SwitchToState (ThreeGppHttpClient::State_t state)
{
  ...
  if ((state == EXPECTING_MAIN_OBJECT) || (state == EXPECTING_EMBEDDED_OBJECT))
    {
      if (m_objectBytesToBeReceived > 0 && !m_streamingMode)
        {
