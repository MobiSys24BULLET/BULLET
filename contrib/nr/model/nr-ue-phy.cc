...
NrUePhy::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::NrUePhy")
            ...
            .AddTraceSource("VideoFrameTrace",
                            "Video application information tracing from the link layer",
                            MakeTraceSourceAccessor(&NrUePhy::m_videoFrameTrace),
                            "ns3::NrUePhy::VideoFrameTracedCallback");
    return tid;
}
...
NrUePhy::UlData(const std::shared_ptr<DciInfoElementTdma>& dci)
{
    ...
    //NS_LOG_UNCOND(Simulator::Now().GetSeconds()<<" Ul current slot"<<m_currentSlot);
    ...
    NS_LOG_INFO("UE" << m_rnti << " TXing UL DATA frame for"
                          ...
                          const Time& duration)
{
    if (pb->GetNPackets() > 0)
    {
        ...
        //ByteTagIterator i = pb->GetPackets().front()->GetByteTagIterator();

        TypeId tid = app_tag.GetInstanceTypeId();
        if (pb->GetPackets().front()->FindFirstMatchingByteTag(app_tag)){
            /*
            while (i.HasNext())
            {
                ...
                {
                    item.GetTag(app_tag);
                    //std::cout<<"UE PHY TX: "<<app_tag.m_frameNum <<" "<< app_tag.m_currentSequence<<std::endl;
                }
            }
            */
            m_videoFrameTrace (app_tag.m_frameNum, app_tag.m_byteSequenceNum, app_tag.m_currentSequence, Simulator::Now() - app_tag.m_senderTimestamp);
        }
        

        /*
        if(pb->GetPackets().front()->FindFirstMatchingByteTag(app_tag)){
            m_videoFrameTrace (app_tag.m_frameNum, app_tag.m_currentSequence, Simulator::Now() - app_tag.m_senderTimestamp);
            //std::cout<<"UE PHY TX: "<<app_tag.m_frameNum <<" "<< app_tag.m_currentSequence<<std::endl;
        }
        */
