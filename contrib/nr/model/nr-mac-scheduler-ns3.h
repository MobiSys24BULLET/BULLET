...
class NrMacSchedulerNs3 : public NrMacScheduler
{
  public:
    ...
    // BULLET
    void IdealBsrTracer (uint16_t rnti, uint32_t bufferSize);  

    void IdealTxTracer (uint16_t rnti, uint32_t txSize);

    mutable std::map <uint16_t, uint32_t> m_idealBsrMap;
    mutable std::map <uint16_t, uint32_t> m_idealTxMap;
    ...
    typedef void (*pBSRTracedCallback)(uint32_t pBSRSize, uint16_t rnti);
    TracedCallback<uint32_t, uint16_t> m_pBSRTrace; //!< Trace callback for Proactive BSR

    typedef void (*txModeTracedCallback)(int transmit_mode, uint16_t rnti);
    TracedCallback<int, uint16_t> m_txModeTrace; //!< Trace callback for Proactive BSR

    typedef void (*ackTracedCallback)(int pdcp_ack, uint16_t rnti);
    TracedCallback<int, uint16_t> m_ackTrace; //!< Trace callback for Proactive BSR

    // Trace from Trigger module

    typedef void (*PreprocessTracedCallback)(uint16_t rnti, int sr, int bsr, bool get_bsr, int alloc, int ack, int totalBuffer);
    TracedCallback<uint16_t, int, int, bool, int, int, int> m_triggerPreprocess; //!< Trace callback for preprocessed input of Trigger

    typedef void (*ProfilingTracedCallback)(uint16_t rnti, int arrival, int mode, double period_app);
    TracedCallback<uint16_t, int, int, double> m_triggerProfile; //!< Trace callback for profiling results of Trigger

    typedef void (*OutputTracedCallback)(uint16_t rnti, int estimated_arrival, int proactive_bsr);
    TracedCallback<uint16_t, int, int> m_triggerOutput; //!< Trace callback for proactive BSR of Trigger

    typedef void (*PeriodTracedCallback)(uint16_t rnti, double eventTime, double length);
    TracedCallback<uint16_t, double, double> m_onPeriodTrace; 
    TracedCallback<uint16_t, double, double> m_offPeriodTrace; 

    mutable std::map <uint16_t, double> m_spectralEfficiencyMap;
    typedef void (*RntiRbMapTracedCallback)(std::map <uint16_t, uint32_t>,std::map <uint16_t, uint32_t>,  
        std::map <uint16_t, uint32_t>,  std::map <uint16_t, uint32_t>, std::map <uint16_t, double>);
    TracedCallback<std::map <uint16_t, uint32_t>, std::map <uint16_t, uint32_t>,  std::map <uint16_t, uint32_t>,  
        std::map <uint16_t, uint32_t>, std::map <uint16_t, double>> m_tracedRntiRbMap; 

    void CallTracedRntiRbMap(const std::map<uint16_t, uint32_t>& allocated_map, const std::map<uint16_t, uint32_t>& proactive_map) const;

    void TriggerPreprocess (uint16_t rnti, int sr, int bsr, bool get_bsr, int alloc, int ack, int totalBuffer);
    void TriggerProfile (uint16_t rnti, int arrival, int mode, double period_app);
    void TriggerOutput (uint16_t rnti, int estimated_arrival, int proactive_bsr);
    void TriggerOnPeriod (uint16_t rnti, double eventTime, double length);
    void TriggerOffPeriod (uint16_t rnti, double eventTime, double length);
    
  protected:
    
    int m_scheduleMode; // 0: Reactive, 1: Grant-free, 2: Proactive BSR, 3: BULLET
    double m_betaVal = 0.9; // parameter for confidence-based resource allocator
    /**
    ...
    // BULLET to set proactive BSR
    virtual void SetProactiveBSR(std::map <uint16_t, ns3::Ptr<BulletTrigger>>) const = 0;
  ...
  private:
    ...
    mutable std::map <uint16_t, ns3::Ptr<BulletTrigger>> m_triggerMap;
