                                         ...
                                         const Ptr<const NrAmc>& amc)
{
    ...
    NS_LOG_DEBUG(Simulator::Now().GetSeconds()<<" Update UL PF Metric for UE "
...
void
NrMacSchedulerUeInfoPF::UpdateUlPFMetricPro(const NrMacSchedulerNs3::FTResources& totAssigned,
                                         double timeWindow,
                                         const Ptr<const NrAmc>& amc)
{
    NS_LOG_FUNCTION(this);

    NrMacSchedulerUeInfo::UpdateUlMetricPro(amc);

    m_currTputUlPro = static_cast<double>(m_ulTbSizePro) / (totAssigned.m_sym);
    m_avgTputUlPro = ((1.0 - (1.0 / static_cast<double>(timeWindow))) * m_lastAvgTputUlPro) +
                  ((1.0 / timeWindow) * m_currTputUlPro);

    NS_LOG_DEBUG(Simulator::Now().GetSeconds()<<" Update UL PF Metric for UE "
                 << m_rnti << " UL TBS: " << m_ulTbSizePro << " Updated currTputUl " << m_currTputUlPro
                 << " avgTputUl " << m_avgTputUlPro << " over n. of syms: " << +totAssigned.m_sym
                 << ", last Avg TH Ul " << m_lastAvgTputUlPro << " total sym assigned "
                 << static_cast<uint32_t>(totAssigned.m_sym)
                 << " updated UL metric: " << m_potentialTputUl / std::max(1E-9, m_avgTputUlPro));
}
