...
class NrMacSchedulerUeInfoPF : public NrMacSchedulerUeInfo
{
  public:
    ...
    void ResetUlSchedInfo() override
    {
        ...
        m_lastAvgTputUlPro = m_avgTputUlPro;
        m_avgTputUlPro = 0.0;
        m_currTputUlPro = 0.0;
        m_potentialTputUlPro = 0.0;
    ...
    void UpdateUlPFMetricPro(const NrMacSchedulerNs3::FTResources& totAssigned,
                          double timeWindow,
                          const Ptr<const NrAmc>& amc);
    ...
    static bool CompareUeWeightsUlPro(const NrMacSchedulerNs3::UePtrAndBufferReq& lue,
                                   const NrMacSchedulerNs3::UePtrAndBufferReq& rue)
    {
        auto luePtr = dynamic_cast<NrMacSchedulerUeInfoPF*>(lue.first.get());
        auto ruePtr = dynamic_cast<NrMacSchedulerUeInfoPF*>(rue.first.get());
        ...
        double lPfMetric = std::pow(luePtr->m_potentialTputUl, luePtr->m_alpha) /
                           std::max(1E-9, luePtr->m_avgTputUlPro);
        double rPfMetric = std::pow(ruePtr->m_potentialTputUl, ruePtr->m_alpha) /
                           std::max(1E-9, ruePtr->m_avgTputUlPro);
    ...
    double m_currTputUlPro{0.0};      //!< Current slot throughput in uplink
    double m_avgTputUlPro{0.0};       //!< Average throughput in uplink during all the slots
    double m_lastAvgTputUlPro{0.0};   //!< Last average throughput in uplink
    double m_potentialTputUlPro{0.0}; //!< Potential throughput in uplink in one assignable resource
                                   //!< (can be a symbol or a RBG)
};
