...
// BULLET
void
NrMacSchedulerNs3::IdealBsrTracer (uint16_t rnti, uint32_t bufferSize)
{
    m_idealBsrMap[rnti] = bufferSize;
}

void
NrMacSchedulerNs3::IdealTxTracer (uint16_t rnti, uint32_t bufferSize)
{
    m_idealTxMap[rnti] = bufferSize;
}
        ...
        TypeId("ns3::NrMacSchedulerNs3")
            ...
            .AddAttribute("ScheduleMode",
                          "Uplink scheduilng mode for resource allocation --> 0: reactive, 1: grant-free, 2: proactive, 3: BULLET, 4: Ideal",
                            IntegerValue (0),
                            MakeIntegerAccessor (&NrMacSchedulerNs3::m_scheduleMode),
                            MakeIntegerChecker<int>(0,10))
            .AddAttribute ("Beta", "Beta value to determine z-score of confidence interval (high value, more conservative approach)",
                    DoubleValue (0.9),
                    MakeDoubleAccessor (&NrMacSchedulerNs3::m_betaVal),
                    MakeDoubleChecker<double>())
            .AddTraceSource("ProactiveBSR",
                            "Proactive BSR from Trigger of BULLET",
                            MakeTraceSourceAccessor(&NrMacSchedulerNs3::m_pBSRTrace),
                            "ns3::NrMacSchedulerNs3::pBSRTracedCallback")  
            .AddTraceSource("TxMode",
                            "Transmit mode distinguishment on Trigger of BULLET",
                            MakeTraceSourceAccessor(&NrMacSchedulerNs3::m_txModeTrace),
                            "ns3::NrMacSchedulerNs3::txModeTracedCallback")
            .AddTraceSource("PdcpAck",
                            "ACK packet traced from PDCP layer",
                            MakeTraceSourceAccessor(&NrMacSchedulerNs3::m_ackTrace),
                            "ns3::NrMacSchedulerNs3::ackTracedCallback")                  
            .AddTraceSource("TriggerPreprocess",
                            "Tracing trigger's preprocessed input",
                            MakeTraceSourceAccessor(&NrMacSchedulerNs3::m_triggerPreprocess),
                            "ns3::NrMacSchedulerNs3::PreprocessTracedCallback")             
            .AddTraceSource("TriggerProfile",
                            "Tracing trigger's profiled intermediate output",
                            MakeTraceSourceAccessor(&NrMacSchedulerNs3::m_triggerProfile),
                            "ns3::NrMacSchedulerNs3::ProfilingTracedCallback")
            .AddTraceSource("TriggerOutput",
                            "Tracing trigger's final output (proactive BSR)",
                            MakeTraceSourceAccessor(&NrMacSchedulerNs3::m_triggerOutput),
                            "ns3::NrMacSchedulerNs3::OutputTracedCallback")
            .AddTraceSource("TriggerOnPeriod",
                            "Tracing trigger's on period",
                            MakeTraceSourceAccessor(&NrMacSchedulerNs3::m_onPeriodTrace),
                            "ns3::NrMacSchedulerNs3::PeriodTracedCallback")
            .AddTraceSource("TriggerOffPeriod",
                            "Tracing trigger's off period",
                            MakeTraceSourceAccessor(&NrMacSchedulerNs3::m_offPeriodTrace),
                            "ns3::NrMacSchedulerNs3::PeriodTracedCallback")
            .AddTraceSource("ResourceBlockTrace",
                            "Resource block tracing to meausre resource waste",
                            MakeTraceSourceAccessor(&NrMacSchedulerNs3::m_tracedRntiRbMap),
                            "ns3::NrMacSchedulerNs3::RntiRbMapTracedCallback");
    ...
    const NrMacCschedSapProvider::CschedUeConfigReqParameters& params)
{
    ...
    if (itUe == m_ueMap.end())
    {
        ...
        // BULLET component
        Ptr<BulletTrigger> bt = CreateObject<BulletTrigger>(params.m_rnti, 4, 8, 3, 5);
        bt->TraceConnectWithoutContext("PreprocessResults", MakeCallback(&ns3::NrMacSchedulerNs3::TriggerPreprocess, this));
        bt->TraceConnectWithoutContext("ProfilingResults", MakeCallback(&ns3::NrMacSchedulerNs3::TriggerProfile, this));
        bt->TraceConnectWithoutContext("OutputResults", MakeCallback(&ns3::NrMacSchedulerNs3::TriggerOutput, this));
        bt->TraceConnectWithoutContext("OnPeriodLength", MakeCallback(&ns3::NrMacSchedulerNs3::TriggerOnPeriod, this));
        bt->TraceConnectWithoutContext("OffPeriodLength", MakeCallback(&ns3::NrMacSchedulerNs3::TriggerOffPeriod, this));
        bt->SetScheduleMode(m_scheduleMode);
        bt->SetBeta(m_betaVal);

        m_triggerMap.insert(std::make_pair(params.m_rnti, bt));
        m_idealBsrMap[params.m_rnti] = 0;


        if (m_ueMap.size() == 1) {
            Config::ConnectWithoutContext(
                "/NodeList/*/DeviceList/*/ComponentCarrierMapUe/*/NrUeMac/BufferTrace",
                MakeCallback(&NrMacSchedulerNs3::IdealBsrTracer, this));
            Config::ConnectWithoutContext(
                "/NodeList/*/DeviceList/*/ComponentCarrierMapUe/*/NrUeMac/TxTrace",
                MakeCallback(&NrMacSchedulerNs3::IdealTxTracer, this));
        }
...
NrMacSchedulerNs3::BSRReceivedFromUe(const MacCeElement& bsr)
{
    ...
    for (uint8_t lcg = 0; lcg < 4; ++lcg)
    {
        ...
        // Trigger tracing
        if (lcg == 2){
            m_triggerMap[bsr.m_rnti]->t_input.bsr = int(bsrId);
            m_triggerMap[bsr.m_rnti]->t_input.get_bsr = true;
        }
        ...
        {
            NS_LOG_UNCOND("Updating UL LCG " << static_cast<uint32_t>(lcg) << " for UE " << bsr.m_rnti
                                           << " size " << bufSize);
        }
        itLcg->second->UpdateInfo(bufSize);
        if (lcg == 2) {
            //m_triggerMap[bsr.m_rnti]->m_proactiveBsr = 0;
        }
    }
}
    ...
    const std::string& direction) const
{
         ...
         /* nothing as increment */)
    {
        ...
        // 230309 for Trigger log acked transport block size
        if (harqFeedbackIt->IsReceivedOk() && direction == "UL"){
            for (std::size_t stream = 0; stream < ueProcess.m_dciElement->m_tbSize.size(); stream++)
            {
               // m_triggerMap[rnti]->gt_tbs = ueProcess.m_dciElement->m_tbSize.at(stream);
            }
        }
                                   ...
                                   const std::string& mode) const
{
    ...
    for (const auto& ueInfo : m_ueMap)
    {
        ...
        //std::cout<<Simulator::Now().GetSeconds()<<" Buffer Active\t";
        ...
        for (const auto& lcgInfo : GetLCGFn(ue))
        {
            ...
            uint32_t proactiveBSR = 0;
            if (lcg->m_id == 2 && mode == "UL") {
                if (m_scheduleMode != 0) {
                    // non-ideal + grant-free
                    if (m_scheduleMode == 1) {
                        if (lcg->GetReactiveTotalSize() == 0) {
                            proactiveBSR = m_triggerMap[ue->m_rnti] -> GetProactiveBSR();
                            //lcg->UpdateInfo (lcg->GetTotalSize() + proactiveBSR);
                            lcg->UpdateProactiveBSR(proactiveBSR);
                        }
                        else {
                            m_triggerMap[ue->m_rnti]->m_proactiveBsr = 0;
                            lcg->UpdateProactiveBSR(proactiveBSR);
                        }
                    }
                    else if (m_scheduleMode == 2 || m_scheduleMode == 3 || m_scheduleMode == 5 || m_scheduleMode == 6) {
                        proactiveBSR = m_triggerMap[ue->m_rnti] -> GetProactiveBSR();
                        lcg->UpdateProactiveBSR(proactiveBSR);
                    } 
                    else {
                        // Update info because the ideal case immediately knows the true buffer size
                        lcg->UpdateInfo (m_idealBsrMap[ue->m_rnti]);
                    }
                }
                m_triggerMap[ue->m_rnti] -> m_totalBuffer = lcg->GetTotalSize();
            }
        ...
        if (totBuffer > 0 && harqV.CanInsert())
        {
            ...
            if (it == activeUe->end())
            {
                // tmp --> vector of UePtrAndBufferReq
                                   ...
                                   uint32_t tbs) const
{
    ...
    NS_LOG_INFO(Simulator::Now().GetSeconds()<<" To distribute: " << tbs << " bytes over " << ueLCG.size() << " LCG");
                                    ...
                                    SlotAllocInfo* slotAlloc) const
{
    ...
    SetProactiveBSR (m_triggerMap);
    //std::cout<<"Schedule at "<<Simulator::Now().GetSeconds()<<std::endl;
    for (auto &kv : m_triggerMap) {
        //kv.second -> TriggerTracing ();
    }
    ...
    std::map <uint16_t, uint32_t> allocated_map;
    std::map <uint16_t, uint32_t> proactive_map;

    for (const auto& el : activeUl)
    {
        // Transform activeuemap to uevector 
        for (const auto& ue : GetUeVector(el))
        {
            uint32_t beamSym = symPerBeam.at(GetUe(el));
            uint32_t rbgAssignable = 1 * beamSym;

            uint16_t temp_rnti = GetUe(ue)->m_rnti;

            uint32_t reactiveBuffer = ue.second - m_triggerMap[temp_rnti] -> GetProactiveBSR();
            uint32_t allocRb = ((GetUe(ue)->m_ulRBG)/rbgAssignable);
            double spectral_efficiency;
            if (allocRb == 0){

            }
            else {
                spectral_efficiency = GetUe(ue)->m_ulTbSize/allocRb;
                m_spectralEfficiencyMap[temp_rnti] = spectral_efficiency;
            }
            int reactiveRb = 0;

            if (spectral_efficiency> 0) {
                //std::cout<<"SE "<< GetUe(ue)->m_ulTbSize<<" "<<allocRb<<" "<<spectral_efficiency<<std::endl;
                reactiveRb = (double)reactiveBuffer/spectral_efficiency;
            }
            int proactiveRb = 0;
            if (ue.second - m_triggerMap[temp_rnti] -> GetProactiveBSR() > 0) {
                int proactiveRb = allocRb - reactiveRb;
                if (proactiveRb < 0) {
                    proactiveRb = 0;
                }
            }
            
            proactive_map[temp_rnti] = proactiveRb;
            allocated_map[temp_rnti] = allocRb;
            ...
            int proactiveAlloc =  GetUe(ue)->m_ulTbSize - reactiveBuffer;
            if (proactiveAlloc <= 0) {
                proactive_map[temp_rnti] = 0;
            }
            else {
                proactive_map[temp_rnti] = (proactiveAlloc)/spectral_efficiency;
            }
        }
    }

    // Trace BS resource allocation
    std::cout<<Simulator::Now().GetSeconds()<<" Scheduled here!"<<std::endl;
    Simulator::Schedule(MicroSeconds(1050), &NrMacSchedulerNs3::CallTracedRntiRbMap, this, allocated_map, proactive_map);
    ...
    for (const auto& beam : activeUl)
    {
        ...
        for (const auto& ue : beam.second)
        {
            ...
            for (const auto& byteDistribution : distributedBytes)
            {
                ...
                NS_LOG_UNCOND("UL LCG " << static_cast<uint32_t>(byteDistribution.m_lcg)
                                       << " assigned bytes " << byteDistribution.m_bytes
                                       << " to LCID "
                                       << static_cast<uint32_t>(byteDistribution.m_lcId));
            }
            std::cout<<"UE " << ue.first->m_rnti<<std::endl;
...
NrMacSchedulerNs3::DoScheduleUlSr(PointInFTPlane* spoint, const std::list<uint16_t>& rntiList) const
{
    ...
    for (const auto& v : rntiList)
    {
        ...
        {
            //NS_LOG_UNCOND("Assigning 12 bytes to UE " << v << " because of a SR" <<ulLcg.second->GetTotalSize());

            uint32_t update_buffer = 12 + ulLcg.second->GetTotalSize() - ulLcg.second->GetReactiveTotalSize();
            
            //std::cout<<Simulator::Now().GetSeconds()<<" Updated buffer SR: "<<update_buffer<<std::endl;
            if (m_scheduleMode != 4) {
                ulLcg.second->UpdateInfo(update_buffer);
            }
            else if (m_scheduleMode == 4) {
                ulLcg.second->UpdateInfo(m_idealBsrMap[v]);
            }
        } 
    }
}
                                ...
                                LteNrTddSlotType type)
{
    ...
    if (activeUlHarq.size() > 0)
    {
        ...
        NS_LOG_DEBUG("For the slot " << ulSfn << " reserved " << static_cast<uint32_t>(usedHarq)
    ...
    const NrMacSchedSapProvider::SchedUlSrInfoReqParameters& params)
{
    ...
    {
        NS_LOG_UNCOND("UE " << ue << " asked for a SR ");

        auto it = std::find(m_srList.begin(), m_srList.end(), ue);
        if (it == m_srList.end())
        {
            m_srList.push_back(ue);
            m_triggerMap[ue]->t_input.sr = 1;
            //m_triggerMap[ue]->gt_sr = Simulator::Now().GetSeconds();
        }
    }
...
void 
NrMacSchedulerNs3::CallTracedRntiRbMap(const std::map<uint16_t, uint32_t>& allocated_map, const std::map<uint16_t, uint32_t>& proactive_map) const
{
    std::cout<<Simulator::Now().GetSeconds()<<" UE resource process in BS Alloc Trace"<<std::endl;
    const std::map <uint16_t, uint32_t> deviceBufferMap = m_idealBsrMap;

    std::map <uint16_t, uint32_t> transmittedRbMap = m_idealTxMap;
    /*
    for (auto &kv : allocated_map) {
        uint16_t rnti = kv.first;
        uint32_t alloc = kv.second;
        uint32_t buffer = deviceBufferMap.at(rnti);
        if (buffer > alloc) {
            transmittedRbMap[rnti] = alloc;
        } else {
            transmittedRbMap[rnti] = buffer;
        }
    }
    */
    const std::map <uint16_t, double> spectralEfficiencyMap = m_spectralEfficiencyMap;

    for (auto &kv : allocated_map) {
        std::cout<<Simulator::Now().GetSeconds()<<" UE resource process in BS Alloc"<< kv.second<<std::endl;
    }

    for (auto &kv : transmittedRbMap) {
        std::cout<<Simulator::Now().GetSeconds()<<" UE resource process in BS Used"<< kv.second<<std::endl;
    }

    m_tracedRntiRbMap(allocated_map, proactive_map, deviceBufferMap, transmittedRbMap, spectralEfficiencyMap);

    for (auto &kv : m_idealTxMap) {
        kv.second = 0;
    }
}
...
void
NrMacSchedulerNs3::TriggerPreprocess (uint16_t rnti, int sr, int bsr, bool get_bsr, int alloc, int ack, int totalBuffer) {
    m_triggerPreprocess (rnti, sr, bsr, get_bsr, alloc, ack, totalBuffer);
}   

void
NrMacSchedulerNs3::TriggerProfile (uint16_t rnti, int arrival, int mode, double period_app) {
    m_triggerProfile (rnti, arrival, mode, period_app);
}

void
NrMacSchedulerNs3::TriggerOutput (uint16_t rnti, int estimated_arrival, int proactive_bsr) {
    m_triggerOutput (rnti, estimated_arrival, proactive_bsr);
}

void
NrMacSchedulerNs3::TriggerOnPeriod (uint16_t rnti, double eventTime, double length) {
    m_onPeriodTrace (rnti, eventTime, length);
}

void
NrMacSchedulerNs3::TriggerOffPeriod (uint16_t rnti,double eventTime, double length) {
    m_offPeriodTrace (rnti, eventTime, length);
}
