        ...
        TypeId("ns3::NrUeMac")
            ...
            .AddAttribute ("SRperiodicity",
                   "Scheduling request periodicity",
                    ...
                    MakeUintegerChecker<uint8_t> ())
            .AddAttribute ("BSRperiodicity", 
                   "Buffer status report periodicity",
                    TimeValue (MilliSeconds(2.5)),
                    MakeTimeAccessor (&NrUeMac::m_bsrPeriodicity),
                    MakeTimeChecker ())
            ...
            .AddTraceSource("RlcBuffer",
                            "Information of RLC buffer.",
                            MakeTraceSourceAccessor(&NrUeMac::m_rlcBufferTrace),
                            "ns3::NrMacRxTrace::RlcBufferTracedCallback")  
            .AddTraceSource("SchedulingRequest",
                            "Scheduling request occurrence.",
                            MakeTraceSourceAccessor(&NrUeMac::m_srTrace),
                            "ns3::TracedValueCallback::Uint32")
            .AddTraceSource("BsrTrace",
                            "BSR and TBS information.",
                            MakeTraceSourceAccessor(&NrUeMac::m_bsrTrace),
                            "ns3::TracedValueCallback::Uint32")
            .AddTraceSource("BufferTrace",
                            "Link buffer size information.",
                            MakeTraceSourceAccessor(&NrUeMac::m_bufferTrace),
                            "ns3::NrMacRxTrace::BufferTraceCallback")
            .AddTraceSource("TxTrace",
                            "Transmitted RB trace",
                            MakeTraceSourceAccessor(&NrUeMac::m_txTrace),
                            "ns3::NrMacRxTrace::BufferTraceCallback")
            .AddTraceSource("SrState",
                            "Current scheduling grant state (0->inactive, 1->active).",
                            MakeTraceSourceAccessor(&NrUeMac::m_srStateTrace),
                            "ns3::TracedValueCallback::Uint32")
            .AddTraceSource("ResourceReceived",
                            "The resource usage/wastage status on UE",
                            MakeTraceSourceAccessor(&NrUeMac::m_resourceTrace),
                            "ns3::TracedValueCallback::ResourceTraceCallback");
    return tid;
}
...
NrUeMac::DoTransmitPdu(LteMacSapProvider::TransmitPduParameters params)
{
    ...
    std::cout<<Simulator::Now().GetSeconds()<<" ulDciTotalUsed increased1: "<<m_rnti<<" "<<m_ulDciTotalUsed<<std::endl;

    // BULLET, preallocation but no packet to transmit
    if (m_miUlHarqProcessesPacket.at(params.harqProcessId).m_pktBurst == nullptr)
    {
        //RefreshHarqProcessesPacketBuffer ();
        //std::cout<<Simulator::Now().GetSeconds()<<" No packet to TX "<<std::endl;
        //return;
        Ptr<PacketBurst> pb = CreateObject<PacketBurst>();
        m_miUlHarqProcessesPacket.at(params.harqProcessId).m_pktBurst = pb;
    }
...
NrUeMac::DoReportBufferStatus(LteMacSapProvider::ReportBufferStatusParameters params)
{
    ...
    //NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " Received BSR for LC Id" << static_cast<uint32_t>(params.lcid));
    // 230328 tracing
    // BSR is reported for each LCG
    std::unordered_map<uint8_t, LteMacSapProvider::ReportBufferStatusParameters>::iterator it2;
    std::vector<uint32_t> queue(4, 0); // one value per each of the 4 LCGs, initialized to 0
    for (it2 = m_ulBsrReceived.begin(); it2 != m_ulBsrReceived.end(); it2++)
    {
        uint8_t lcid = it2->first;
        ...
        NS_ASSERT_MSG((lcid != 0) ||
                        (((*it2).second.txQueueSize == 0) && ((*it2).second.retxQueueSize == 0) &&
                        ((*it2).second.statusPduSize == 0)),
                    "BSR should not be used for LCID 0");
        uint8_t lcg = lcInfoMapIt->second.lcConfig.logicalChannelGroup;
        queue.at(lcg) +=
            ((*it2).second.txQueueSize + (*it2).second.retxQueueSize + (*it2).second.statusPduSize);
        ...
        if ((*it2).second.txQueueSize > 0)
        {
            NS_LOG_DEBUG("Adding 3 bytes for TX subheader.");
            queue.at(lcg) += 3;
        }
        if ((*it2).second.retxQueueSize > 0)
        {
    ...
    m_bufferTrace(m_rnti, queue.at(2));
    ...
    // See UL DCI, and if there are a plenty of resources, do not transmit SR.
    ...
    std::cout<<"MAC front frame: "<< params.front_frameNum <<std::endl;
    m_rlcBufferTrace(params.front_frameNum, params.onDeviceQueueing, params.is_frame);
    m_freshUlBsr = true;
}
...
NrUeMac::SendReportBufferStatus(const SfnSf& dataSfn, uint8_t symStart)
{
    ...
    for (it = m_ulBsrReceived.begin(); it != m_ulBsrReceived.end(); it++)
    {
        ...
        if ((*it).second.is_frame){
            std::cout<<Simulator::Now().GetSeconds()<<" MAC buffer frame "<<(*it).second.front_frameNum<<" "<<(*it).second.front_sequence<<" "
                    <<(*it).second.back_frameNum<<" "<<(*it).second.back_sequence<<std::endl;
        }
    }

    //NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " Sending BSR with this info for the LCG: "
    //            << queue.at(0) << " " << queue.at(1) << " " << queue.at(2) << " " << queue.at(3));

    //queue.at(2) = 10000;
    
    std::cout<<Simulator::Now().GetSeconds()<< " BSR tx "<< dataSfn << " "<< queue.at(2)<<std::endl;

    m_bsrTrace((int)NrMacShortBsrCe::FromBytesToLevel(queue.at(2)), m_ulDci->m_tbSize.at(0));
    m_bufferTrace(m_rnti, queue.at(2));
    ...
    std::cout<<Simulator::Now().GetSeconds()<<" ulDciTotalUsed increased2: "<<m_rnti<<" "<<queue.at(2)<<" "<<m_ulDci->m_tbSize.at(0)<<" "<<m_ulDciTotalUsed<<std::endl;
...
NrUeMac::DoSlotIndication(const SfnSf& sfn)
{
    ...
    {
      if((sfn.GetFrame()*20+ 2*sfn.GetSubframe() + sfn.GetSlot())%m_srPeriodicity==(2%m_srPeriodicity))
      {
        NS_LOG_INFO ("Sending SR to PHY in slot " << sfn<< " "<<(sfn.GetFrame()*10+sfn.GetSubframe()));
...
NrUeMac::SendSR() const
{
    ...
    //std::cout<<Simulator::Now().GetSeconds()<<" "<<"Send SR"<<std::endl;
...
NrUeMac::ProcessUlDci(const Ptr<NrUlDciMessage>& dciMsg)
{
    ...
    uint32_t beforeBuffer = GetTotalBufSize();

    std::cout<<Simulator::Now().GetSeconds()<<" 1 MAC UE Buffer size: "<<m_rnti<<" "<<GetTotalBufSize()<<std::endl;
    ...
    std::cout<<Simulator::Now().GetSeconds()<<" UL dci total used before loop: "<<m_ulDciTotalUsed<<std::endl;
    ...
    if (m_ulDci->m_ndi.at(0) == 0)
    {
        ...
        TransmitRetx();
        std::cout<<Simulator::Now().GetSeconds()<<" UL dci total used after retx: "<<m_ulDciTotalUsed<<std::endl;
        // This method will transmit a new BSR.
         if ((Simulator::Now() >= m_bsrLast + m_bsrPeriodicity))
        {
            //NS_LOG_UNCOND(Simulator::Now().GetSeconds()<< " Hi2");
            SendReportBufferStatus(dataSfn, m_ulDci->m_symStart);
            m_bsrLast = Simulator::Now();
            m_freshUlBsr = false;
        }
        std::cout<<Simulator::Now().GetSeconds()<<" 2 MAC UE Buffer size: "<<m_rnti<<" "<<GetTotalBufSize()<<std::endl;
        std::cout<<Simulator::Now().GetSeconds()<<" UL dci total used after retx2: "<<m_ulDciTotalUsed<<std::endl;
    }
    else if (m_ulDci->m_ndi.at(0) == 1)
    {
        ...
        //NS_LOG_UNCOND("UL DCI processing done, sent to PHY a total of "
        //           << m_ulDciTotalUsed << " B out of " << m_ulDci->m_tbSize.at(0)
        //           << " allocated bytes ");
        std::cout<<Simulator::Now().GetSeconds()<<" 3 MAC UE Buffer size: "<<m_rnti<<" "<<GetTotalBufSize()<<std::endl;

        if (GetTotalBufSize() == 0)
        {
            m_srState = INACTIVE;
            m_srStateTrace(0);
            NS_LOG_UNCOND("ACTIVE -> INACTIVE, bufSize " << GetTotalBufSize());
            ...
            {
                //NS_LOG_UNCOND(Simulator::Now().GetSeconds()<< " No byte used for this UL-DCI, sending empty PDU");
    ...
    uint32_t afterBuffer = GetTotalBufSize();
    int numRb = 0;
    int wastedRb = 0;
    double bitsPerRb = 0;
    int usedRb = 0;
    for (const auto &value : m_ulDci->m_rbgBitmask) {
        numRb += static_cast<int>(value);
    }
    if (numRb == 0) {
        bitsPerRb = 0;
        usedRb = 0;
        ...
        bitsPerRb = (double)m_ulDci->m_tbSize.at(0)/(double)numRb;
        if (bitsPerRb != 0) {
            usedRb = (m_ulDciTotalUsed + bitsPerRb - 1)/bitsPerRb;
        }
        else {
            usedRb = 0;
        }
    }
    usedRb = std::min(usedRb, numRb);

    wastedRb = numRb - usedRb;
    
    m_txTrace (m_rnti, usedRb);
    m_resourceTrace(m_rnti, numRb, usedRb, wastedRb);

    
    //std::cout<<Simulator::Now().GetSeconds()<<" 2 MAC UE Buffer size: "<<m_rnti<<" "<<beforeBuffer<<" "<<afterBufferstd::endl;

    std::cout<<Simulator::Now().GetSeconds()<<" UE resource process "<<m_rnti<<" Alloc TBS: "<<m_ulDci->m_tbSize.at(0)<<
        " Alloc RB: "<<numRb<<" Used: "<<m_ulDciTotalUsed<<" bits per RB: "<<bitsPerRb<<" Wasted RB:" <<wastedRb<<std::endl;
...
NrUeMac::TransmitRetx()
{
    ...
    //NS_LOG_UNCOND(Simulator::Now().GetSeconds()<<" HARQ process in retx "<<+m_ulDci->m_harqProcess);
    ...
    for (std::list<Ptr<Packet>>::const_iterator j = pb->Begin(); j != pb->End(); ++j)
    {
        ...
        // one stream with stream Id 0.
        uint8_t streamId = 0;
        m_ulDciTotalUsed += pkt->GetSize();
        std::cout<<Simulator::Now().GetSeconds()<<" ulDciTotalUsed increased3: "<<m_rnti<<" "<<m_ulDciTotalUsed<<std::endl;
...
NrUeMac::SendRetxData(uint32_t usefulTbs, uint32_t activeLcsRetx)
{
    ...
    for (auto& itBsr : m_ulBsrReceived)
    {
        ...
        if (m_ulDciTotalUsed + bytesPerLcId <= m_ulDci->m_tbSize.at(0))
        {
...
NrUeMac::SendTxData(uint32_t usefulTbs, uint32_t activeTx)
{
    ...
    for (auto& itBsr : m_ulBsrReceived)
    {
        ...
        std::cout<<Simulator::Now().GetSeconds()<<" "<<m_rnti<<" TxNewData: "<<m_ulDciTotalUsed << " "<< bytesPerLcId<<" "<<m_ulDci->m_tbSize.at(0)<<std::endl;
        ...
        if (m_ulDciTotalUsed + bytesPerLcId <= m_ulDci->m_tbSize.at(0))
        {
            ...
            NS_LOG_UNCOND(Simulator::Now().GetSeconds()<<" (1) Notifying RLC of LCID " << +bsr.lcid
                                                 << " of a TxOpp "
                                                    "of "
...
{
    NS_LOG_FUNCTION(this);
    //NS_LOG_UNCOND(Simulator::Now().GetSeconds()<<" Send new data: "<< m_ulDciTotalUsed);
    ...
    //NS_LOG_UNCOND(Simulator::Now().GetSeconds()<<" Reset HARQP " << +m_ulDci->m_harqProcess);
    ...
    //std::cout<<Simulator::Now().GetSeconds()<<" userful TBS: "<<usefulTbs<<" activeLcsTx: "<<activeLcsTx<<std::endl;
    ...
    if (m_ulDciTotalUsed == 0)
    {
        //NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " pktBurst Clear");
    ...
    for (auto& bsrIt : m_ulBsrReceived)
    {
        ...
        if (bsr.statusPduSize > 0)
        {
            ...
            if (m_ulDciTotalUsed + bsr.statusPduSize <= m_ulDci->m_tbSize.at(0))
            {
                ...
                NS_LOG_UNCOND("Notifying RLC of LCID " << +bsr.lcid
                                                     << " of a TxOpp "
                                                        "of "
