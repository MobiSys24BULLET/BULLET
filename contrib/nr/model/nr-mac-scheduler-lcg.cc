...
{
    // modified for BULLET
    //std::cout<<"Updated: "<<m_rlcTransmissionQueueSize<<" "<< m_proactiveQueueSize<<std::endl;
    return m_rlcTransmissionQueueSize + m_rlcRetransmissionQueueSize + m_rlcStatusPduSize + m_proactiveQueueSize;
}

uint32_t
NrMacSchedulerLC::GetReactiveTotalSize() const
{
    // modified for BULLET
...
NrMacSchedulerLCG::UpdateInfo(uint32_t lcgQueueSize)
{
    ...
    //std::cout<<Simulator::Now().GetSeconds()<<" BSR in Scheduler (LCG): "<<lcIdPart<<" "<<lcgQueueSize<<std::endl;
...
void
NrMacSchedulerLCG::UpdateProactiveBSR (uint32_t proactiveBSR)
{
    NS_LOG_FUNCTION(this);
    NS_ABORT_IF(m_lcMap.size() > 1);
    uint32_t lcIdPart = proactiveBSR / m_lcMap.size();

    for (auto& lc: m_lcMap) 
    {
        lc.second->m_proactiveQueueSize = lcIdPart;
    }
}
...
uint32_t
NrMacSchedulerLCG::GetReactiveTotalSize() const
{
    NS_LOG_FUNCTION(this);
    uint32_t totalSize = 0;
    for (const auto& lc : m_lcMap)
    {
        totalSize += lc.second->GetReactiveTotalSize();
    }
    NS_LOG_INFO("Total size: " << totalSize);
    return totalSize;
}
...
// resource allocation occurs here
void
NrMacSchedulerLCG::AssignedData(uint8_t lcId, uint32_t size, std::string type)
{
                ...
                // data to be transmitted
    {
        ...
        if (size - rlcOverhead >= m_lcMap.at(lcId)->m_rlcTransmissionQueueSize)
        {
            ...
            // this line prevents the next transmission when all HARQ retx fails. However, note that we can still make lcg queue as 0 with zero BSR reception
            m_lcMap.at(lcId)->m_rlcTransmissionQueueSize = 50; 
        }
        else
        {
