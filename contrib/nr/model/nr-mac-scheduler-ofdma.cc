...
NrMacSchedulerOfdma::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::NrMacSchedulerOfdma")
            ...
            .AddAttribute ("BBR", "Enable buffer-based resource allocation",
                    BooleanValue (true),
                    MakeBooleanAccessor (&NrMacSchedulerOfdma::m_enableBbr),
                    MakeBooleanChecker())
            .AddTraceSource(
                "SymPerBeam",
...
NrMacSchedulerOfdma::AssignULRBG(uint32_t symAvail, const ActiveUeMap& activeUl) const
{
    ...
    // Iterate through the different beams (activeUl --> beam)
    for (const auto& el : activeUl)
    {
        ...
        // Calculate potential throughput of all users in PF scheduler (maybe calculate scheduler metrics to optimize)
        for (auto& ue : ueVector)
        {
            // FTResources --> amount of resources that can be allocated in frequecny, time domain
            BeforeUlSched(ue, FTResources(rbgAssignable * beamSym, beamSym));
        }

        int max_iter = 1;
        if (m_scheduleMode == 3) { // BULLET adopts multi-stage robust optimization
            max_iter = 2;
        }
        uint32_t proactiveQueueSize = 0;
        // RNTI, allocated 

        std::map <uint16_t, uint32_t> allocated_map;
        std::map <uint16_t, uint32_t> proactive_map;
        std::map <uint16_t, uint32_t> reactiveBuffer_map;
        std::map <uint16_t, uint32_t> bufferMap;
        GetFirst GetUe;
        for (const auto& ue : ueVector) {
            uint16_t rnti = GetUe(ue)->m_rnti;
            reactiveBuffer_map[rnti] = ue.second;
        }

        uint32_t prev_resources;
        ...
        for (int i = 0; i < max_iter; i++) 
        {
            ...
            std::cout<<Simulator::Now().GetSeconds()<<" iteration: "<<i<<" "<<resources<<std::endl;
            if (resources == 0) {
                break;
            }
            else {
                std::cout<<"Booster working at "<<i<<" "<<resources<<" "<<prev_resources<<std::endl;
            }

            //GetFirst GetUe;
            // Iterate using a for loop
            //std::cout<<Simulator::Now().GetSeconds()<<" Buffer Status! \t";
            for (const auto& ue : ueVector) {
                uint32_t allocated_tbs = GetUe(ue)->m_ulTbSize;
                allocated_map[GetUe(ue)->m_rnti] = allocated_tbs;
                uint32_t buf = ue.second;
                newBufferSize = (int)buf - (int)allocated_tbs;
                if (newBufferSize < 0) {
                    newBufferSize = 0;
                }
                bufferMap[GetUe(ue)->m_rnti] = newBufferSize;
                std::cout<<GetUe(ue)->m_rnti<<" Resource allocation "<<buf<<" "<<allocated_tbs<<std::endl; 
            }
            //std::cout<<std::endl;
            prev_resources = resources;
            bool m_robust = false;
            if (i >=1 && m_scheduleMode == 3) {
                m_robust = false;
            }
            ...
            if (m_scheduleMode == 1) {
                for (const auto& ue : ueVector) {
                    uint16_t rnti = GetUe(ue)->m_rnti;
                    if (m_proactiveBSR[rnti] > 0) {
                        while (GetUe(ue)->m_ulRBG/rbgAssignable < 15) {
                            // Check assigned resources
                            GetUe(ue)->m_ulRBG += rbgAssignable;
                            assigned.m_rbg += rbgAssignable;

                            // Check assigned beamSym
                            GetUe(ue)->m_ulSym = beamSym;
                            assigned.m_sym = beamSym;
            ...
            while (resources > 0)
            {
                // Compared users with resource allocation metrics
                //GetFirst GetUe;
                if (!m_robust) {
                    std::sort(ueVector.begin(), ueVector.end(), GetUeCompareUlFn());
                }
                else {
                    std::sort(ueVector.begin(), ueVector.end(), GetUeCompareUlFnPro());
                }
                auto schedInfoIt = ueVector.begin(); // Schedule user with the highest priority
                double buffer_penalty = 1;

                // Ensure fairness: pass over UEs which already has enough resources to transmit
                // Schedule with the prioirty calculated before
                while (schedInfoIt != ueVector.end())
                {
                    //uint32_t bufQueueSize = schedInfoIt->second;
                    ...
                    if ((m_scheduleMode == 3) && i == 0) {
                        bufQueueSize = m_confidentBSR[rnti];
                    }

                    std::cout<<Simulator::Now().GetSeconds()<<" BufQueueSize: "<<rnti<<" "<<bufQueueSize<<" "<<m_proactiveBSR[rnti]<<" "<<m_confidentBSR[rnti]<<" "<<schedInfoIt->second<<" "<<i<<" "<<
                     GetUe(*schedInfoIt)->m_ulTbSize<<std::endl;

                    if (m_enableBbr){
                        if (Simulator::Now().GetMilliSeconds() - m_bufferResetTime.GetMilliSeconds() > m_timeWindow)
                        {
                            m_maxBuffer = 0;
                        }
                        if (m_maxBuffer == 0)
                        {
                            m_maxBuffer = bufQueueSize;
                        }
                        else
                        {
                            m_maxBuffer = 0.2*bufQueueSize + 0.8*m_maxBuffer;
                        }
                        buffer_penalty = std::max((double)m_maxBuffer-60000, (double)0)/(double)150000 + 0.4;
                        //buffer_penalty = std::min(1.0, buffer_penalty);
                    } 
                        
                    //std::cout<<Simulator::Now().GetSeconds()<<" Updated buffer: "<<bufQueueSize<<" "<<GetUe(*schedInfoIt)->m_ulTbSize <<std::endl;
                    
                    //std::cout<<Simulator::Now().GetSeconds()<<" Updated buffer: "<<bufQueueSize<<" "<<GetUe(*schedInfoIt)->m_ulTbSize <<std::endl;
                    // If the allocated resources is higher than the buffer (or penaltyed buffer), then schedule next user
                    if (GetUe(*schedInfoIt)->m_ulTbSize >= std::max(bufQueueSize, 12U)|| (GetUe(*schedInfoIt)->m_ulRBG/rbgAssignable) >= max_resources*buffer_penalty)
                    {
                        schedInfoIt++;
                        //std::cout<<Simulator::Now().GetSeconds()<<" BSR in scheduler: "<<bufQueueSize<<std::endl;
                    }
                    else
                    {
                        break;
                ...
                if (schedInfoIt == ueVector.end())
                {
                    // 230419: Break if the input buffer is not left.. then we re-input buffer from next round  
                    break;
                }

                // Assign 1 RBG for each available symbols for the beam,
                // and then update the count of available resources
                NS_LOG_DEBUG(Simulator::Now().GetSeconds()<<" nPRB "<< GetUe(*schedInfoIt)->m_ulRBG 
                    << " "<< rbgAssignable<< " " << resources<< " "<< GetUe(*schedInfoIt)->m_ulTbSize<< " bufSize: "<< schedInfoIt->second);
                ...
                // Resource allocation for proactive BSR
                if (m_robust) {
                    GetUe(*schedInfoIt)->m_ulRBGPro += rbgAssignable;
                }
                ...
                if (!m_robust) {
                    ...
                    NS_LOG_DEBUG("Assigned " << rbgAssignable << " UL RBG, spanned over " << beamSym
                                            << " SYM, to UE " << GetUe(*schedInfoIt)->m_rnti);
                    AssignedUlResourcesPro(*schedInfoIt, FTResources(rbgAssignable, beamSym), assigned);
                    ...
                    for (auto& ue : ueVector)
                    {
                        if (GetUe(ue)->m_rnti != GetUe(*schedInfoIt)->m_rnti)
                        {
                            NotAssignedUlResourcesPro(ue, FTResources(rbgAssignable, beamSym), assigned);
                        }
                    }
                }
            }
...
void
NrMacSchedulerOfdma::SetProactiveBSR (std::map <uint16_t, ns3::Ptr<BulletTrigger>> triggerMap) const
{
    //std::cout<<"Mode: "<<m_scheduleMode<<std::endl;
    std::map<uint16_t, ns3::Ptr<BulletTrigger>>::iterator it;
    for (it = triggerMap.begin(); it != triggerMap.end(); it++) {
        uint16_t rnti = it->first;
        m_proactiveBSR[rnti] = it->second->GetProactiveBSR();
        m_confidentBSR[rnti] = it->second->GetConfidentBSR();
    }   
}
                                 ...
                                 uint32_t maxSym) const
{
    ...
    //NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " DCI nPRB "<< ueInfo->m_ulRBG <<" "<<GetNumRbPerRbg()); 
