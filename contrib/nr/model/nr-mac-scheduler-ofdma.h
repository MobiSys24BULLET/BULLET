...
class NrMacSchedulerOfdma : public NrMacSchedulerTdma
{
  ...
  mutable Time m_bufferResetTime;
  mutable uint32_t m_timeWindow = 20;
  mutable uint32_t m_maxBuffer = 0;

  mutable std::map <uint16_t, uint32_t> m_proactiveBSR;
  mutable std::map <uint16_t, uint32_t> m_confidentBSR;

  double m_betaVal = 0;
  //double m_beta[4] = {m_betaVal, 1};
