...
NrAmc::CalculateTbSize(uint8_t mcs, uint32_t nprb) const
{
    ...
    //std::cout<<Simulator::Now().GetSeconds()<<" MCS: "<<static_cast<uint32_t>(mcs)<< " maximum MCS: "<<static_cast<uint32_t>(m_errorModel->GetMaxMcs())<<std::endl;
    ...
    /*
    NS_LOG_UNCOND(Simulator::Now().GetSeconds()<< " mcs:" << (unsigned)mcs << 
    " TB size:" << tbSize << " payload: " << payloadSize<< " NPRB: " << nprb);
    */
...
NrAmc::CreateCqiFeedbackWbTdma(const SpectrumValue& sinr, uint8_t& mcs) const
{
    ...
    else if (m_amcModel == ErrorModel)
    {
        ...
        while (mcs <= m_errorModel->GetMaxMcs())
        //while (mcs <= m_lastMcs + 2 && mcs <= m_errorModel->GetMaxMcs())
        {
        ...
        NS_LOG_INFO(this << "\t MCS " << (uint16_t)mcs << "-> CQI " << cqi);
    }
    m_lastMcs = mcs;
    return cqi;
}
