...
class NrUeMac : public Object
{
  ...
  public:
    ...
    typedef void (*RlcBufferTracedCallback)(int bufferSize,
                                            double onDeviceQueuing, 
                                            bool is_frame);

    typedef void (*BufferTraceCallback)(uint16_t rnti, uint32_t buffer);
    TracedCallback<uint16_t, uint32_t> m_bufferTrace;
    TracedCallback<uint16_t, uint32_t> m_txTrace;

    typedef void (*ResourceTraceCallback)(uint16_t rnti, uint32_t allocRb, uint32_t usedRb, uint32_t wastedRb);
    TracedCallback <uint16_t, uint32_t, uint32_t, uint32_t> m_resourceTrace;
  ...
  private:
    ...
    TracedCallback<int, double, bool>
        m_rlcBufferTrace;
    

    TracedCallback<uint32_t> m_srTrace;

    TracedCallback<uint32_t, uint32_t> m_bsrTrace;

    TracedCallback<uint32_t> m_srStateTrace;
};
