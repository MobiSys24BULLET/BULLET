    ...
    static TypeId tid =
        TypeId("ns3::NrGnbMac")
            ...
            .AddTraceSource("RxPdu",
                            "Receive PDU size from PHY layer.",
                            MakeTraceSourceAccessor(&NrGnbMac::m_rxPdu),
                            "ns3::NrGnbMac::RxPduTracedCallback");
    return tid;
}
...
NrGnbMac::DoReceivePhyPdu(Ptr<Packet> p)
{
    ...
    //std::cout<<Simulator::Now().GetSeconds()<<" Phy PDU rx: "<<p->GetSize()<<" "<<rnti<<std::endl;
    ...
    if (header.GetLcId() == NrMacHeaderFsUl::SHORT_BSR)
    {
        ...
        //NS_LOG_UNCOND(Simulator::Now().GetSeconds()<<" BSR rx "<<bsrHeader.m_bufferSizeLevel_2);
    ...
    std::cout<<Simulator::Now().GetSeconds()<<" MAC Rx PDU: "<<p->GetSize()<<std::endl;
    m_rxPdu(p->GetSize(), rnti);
