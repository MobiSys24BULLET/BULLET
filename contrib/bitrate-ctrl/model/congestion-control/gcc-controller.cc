...
const double kOverUsingTimeThreshold = 100; // key parameter
...
const int kDefaultMaxBitrateBps = 30e6;
...
GccController::GccController(uint16_t port) :
    ...
    min_configured_bitrate_bps_(3e6), //initial value need
    max_configured_bitrate_bps_(30*1e6),
...
void GccController::CapBitrateToThresholds(int64_t now_ms,
  ...
  std::cout<<ns3::Simulator::Now().GetSeconds()<<" delay: "<<delay_based_bitrate_bps_<<" bitrate: "<< bitrate_bps<<std::endl;
...
bool GccController::processFeedback(uint64_t nowUs,
  ...
  std::cout<<ns3::Simulator::Now().GetSeconds()<<" Gcc Rtt: "<<rtt_clamped<<std::endl; 
  ...
  if (!update_estimate) {
    ...
    // of an over-use.
      ...
      //<<"PORT: "<<this->m_port<<" " <<"esti case1"<<std::endl;
  ...
  // and the target bitrate is too high compared to what we are receiving.	
	...
	if (update_estimate){	
    //std::cout<<ns3::Simulator::Now().GetSeconds()<<" m_RecvR: "<< (uint32_t)m_RecvR<<" "<<current_bitrate_bps_<<std::endl;
...
uint32_t GccController::Update(char bw_state, uint32_t incoming_bitrate, double noise_var,
  ...
  //std::cout<<ns3::Simulator::Now().GetSeconds()<<" Gcc update "<<incoming_bitrate<<" "<<now_ms<<" "<<now_ms - time_first_incoming_estimate_ <<" "<<bitrate_is_initialized_ <<std::endl; 

  if (!bitrate_is_initialized_) {
    const int64_t kInitializationTimeMs = 500;
...
void GccController::logStats(uint64_t nowUs) const {
    ...
    std::ofstream f_r("results/SendRate"+std::to_string(this->m_port)+".txt", std::ios::app);
    std::ofstream f_fl("results/FractionLoss.txt", std::ios::app);
    std::ofstream f_rtt("results/RTT"+std::to_string(this->m_port)+".txt", std::ios::app);

    if(!f_r || !f_t || !f_fl || !f_rtt){
      std::cout<<"File open error"<<std::endl;
    }
    else{
...
char GccController::OveruseDetectorDetect(double offset, double ts_delta, int num_of_deltas, int64_t now_ms){
  ...
  std::cout<<" Threshold: "<< threshold_ <<" "<< T<<" "<<D_hypothesis_<<std::endl;
