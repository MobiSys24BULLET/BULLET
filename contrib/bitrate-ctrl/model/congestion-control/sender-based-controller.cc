...
bool SenderBasedController::processSendPacket(uint64_t txTimestampUs,
    ...
    for (auto it = m_PacketTransitHistory.begin(); it != m_PacketTransitHistory.end(); it++) {
        if (it->sequence == m_lastSequence) {
            m_PacketTransitHistory.erase(it);
        }
    }
...
bool SenderBasedController::processFeedback(uint64_t nowUs,
    ...
    std::cout<<this<<" RTT why: "<<ns3::Simulator::Now().GetSeconds()<<" "<<packet.rttUs<<std::endl;
...
bool SenderBasedController::getCurrentRecvRate(float& rrateBps) {
    ...
    // std::cout << "recvhistory: " << " lastrxus: " << lastRxUs << " ";
    while ((lastRxUs - curr) < 1000000){
        ...
        // std::cout << "[" << i << " " << m_recvHistory.at(i).size << " " << curr << "]";
        if (i == 0)
            return false;
        i--;
    }
